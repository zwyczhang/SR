...
def select_dataset(data_dir, mode):
    if mode == "test":
        ...
        path = []
        for idx, path_name in enumerate(sorted(glob.glob(os.path.join(os.path.join(data_dir, 'HR'), '*')))):
            (imgname, imgext) = os.path.splitext(os.path.basename(path_name))


            path.append((os.path.join(data_dir, f"X4/{imgname}.png"),
                    path_name))
            # path = [
            #     (
            #         os.path.join(data_dir, f"X4/{imgname}.png"),
            #         path_name
            #     )
            # ]
...
def main(args):
    ...
    if args.include_test:
        ...
        s = fmt.format("Model", "Val PSNR", "Val SSIM", "Val Time [ms]", "Params [M]", "FLOPs [G]", "Acts [M]", "Mem [M]", "Conv")
    for k, v in results.items():
        val_psnr = f"{v['valid_ave_psnr']:2.2f}"
        val_ssim = f"{v['valid_ave_ssim']:2.4f}"
        ...
        if args.include_test:
            ...
            s += fmt.format(k, val_psnr, val_ssim, test_psnr, val_time, test_time, ave_time, num_param, flops, acts, mem, conv)
        else:
            s += fmt.format(k, val_psnr, val_ssim, val_time, num_param, flops, acts, mem, conv)
    data_name = args.data_dir.split('/')[-1]
    print('********************', data_name)
    util.mkdir(os.path.join('D:/NTIRE2022_ESR-main', f'{model_name}'))
    with open(os.path.join(os.getcwd(), f'{model_name}/results_{data_name}.txt'), "w") as f:
        f.write(s)


if __name__ == "__main__":

    data1 = ['Set5', 'Set14', 'BSDS100', 'urban100', 'manga109']
    data2 = ['DIV2K100']

    for i in data2:
        data_dir = f'E:\\testsets/{i}'
        print(data_dir)

        parser = argparse.ArgumentParser("NTIRE2022-EfficientSR")
        parser.add_argument("--data_dir", default=data_dir, type=str)
        parser.add_argument("--save_dir", default="None\\dk", type=str)
